...
const int PIN_LED_GREEN = 2; // LED Bleue intégrée (au lieu de LED externe sur D12)
const int PIN_LED_RED = 25;   // Libre
...
void setup() {
  ...
  // Setup Afficheur 3461BS (12 pins)
  byte numDigits = 4;
  
  // Mapping ESP32 -> Segments (A, B, C, D, E, F, G, DP)
  byte segmentPins[] = {15, 2, 0, 4, 16, 17, 5, 18}; 
  
  // Mapping ESP32 -> Digits (D1, D2, D3, D4)
  byte digitPins[] = {19, 21, 22, 23}; 
  
  bool resistorsOnSegments = true; 
  byte hardwareConfig = COMMON_ANODE; // "BS" signifie souvent Common Anode
...
void checkFirebase() {
  ...
  if (isCharging) {
    ...
    sevseg.setChars("LIBRE"); // Ou "----"
  }
}

void updateHardware(bool active) {
  if (active) {
    digitalWrite(PIN_LED_RED, LOW);   // Rouge OFF
    digitalWrite(PIN_LED_GREEN, HIGH); // Vert ON
    myServo.write(90); 
  } else {
    digitalWrite(PIN_LED_GREEN, LOW);  // Vert OFF
    digitalWrite(PIN_LED_RED, HIGH);   // Rouge ON
    myServo.write(0); 
  }
}
